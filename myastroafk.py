...
class AstroAfkMod(loader.Module):
	...
	strings = {
		...
		"custom_text__afk_text": "Сейчас я занят 🚫.\nПопробуйте написать позже\n😴Занят уже как:{time}",
	}
	...
	async def goafk(self, message):
		...
		if change_bio:
			...
			if not cfg_bio:
				await message.client(UpdateProfileRequest(about="Сейчас я занят. Не мешайте мне"))
			else:
